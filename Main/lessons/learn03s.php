  ...
  <body>
    ...
    <!--nav Botton-->
      <nav class="navbar navbar-default navbar-fixed-bottom">
          <a class="waves-effect waves-light btn tooltipped" id="backButton" href="learn03.php" data-position="top" data-tooltip="Go back to insecure methods">
            <i class="material-icons left">arrow_back</i>Insecure </a>
          <a class="tooltipped waves-effect waves-light btn tooltipped" id="nextButton" href="learn04.php?id=1" data-position="top"
      ...
      <!--Content-->
	    ...
	    <li id="learn"class="active">
	      ...
	      <div class="collapsible-body" id="textarea">
                <span>
                  There are several ways to resolve this type of illegal login:<br>
<ol>
<li>Filtering strings with regular expressions is the most time-consuming and safest way to do this.
</li>
<li>Use addslashes(); this function will precede all single quotes with a backslash. Although this method is relatively simple, it cannot filter all attacks, but this is a built-in method in the new version of php.
</li>

<li>For MySQL users, the best escaping function is <strong>mysqli_real-escape_string()</strong>. If there is no native escaping function for your database, addslashes() can be used as a resort.
	Here's a simple explanation with mysqli_real-escape_string()
</li>
<br>
//before the received parameter add <strong>mysqli_real-escape_string($link, $escapestr)</strong>
<pre>$account = mysqli_real-escape_string($link, $_REQUEST['account']);

$pass = mysqli_real-escape_string($link, $_REQUEST['password']);</pre>

  <li>Fixed parameter types, such as mysql_fetch_array(), mysql_fetch_assoc() and mysql_fetch_row(), one can be a string or a number, one can only be a number
Use ajax to verify: use one parameter to grab any other parameter and remove the special character. A more detailed introduction to this method will be in the Broken Authentication course.
</li>
</ol>
                </span>
              </div>
                ...
                <span>
                  <ol>
                    <li>Try to attack it by inputting the following instructions in the username input box again.</li>
                  <pre>' OR ''=''#</pre>
                      <li>Then see what will it happen.</li>
                <li>check the code(code comparison). Then turn on the switch and go to the next lesson.</li>
                </span></div>
            </li>
...
<!--example---------------------------------------------->
        <div class="col-md-4" id="example">
          ...
          <div class="switch right" >
            <label>
              Off
              <input type="checkbox" id="switch" onchange="gcheck('switch')" <?php echo ($_SESSION['user_L3'] == 1)?'checked':'';?>>
        ...
        <script>
          $(document).on("ready", function() {
            $("form.login").on("submit", function(){
              ...
              $.ajax({
                type : "POST",
                url : "../php/verify_user_Demo01S.php",
                data : {
                  un : $("#username").val(),
                  pw : $("#password").val()
                },
